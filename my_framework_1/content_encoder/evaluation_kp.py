...
parser.add_argument('--predict_file', type=str, default='result_kp/predict.json')
parser.add_argument('--output_file', type=str, default='result_kp/predict.png')
...
if __name__ == '__main__': 
    ...
    for i in range(n_frames):
        for j in range(n_frames):
            ...
            for p in pred[0,frame_index,:68]:
                image[p[1]-pixel_size:p[1]+pixel_size, p[0]-pixel_size:p[0]+pixel_size,:] = (0, 255, 0)
            for p in pred[0,frame_index,71:72]:
                image[p[1]-pixel_size:p[1]+pixel_size, p[0]-pixel_size:p[0]+pixel_size,:] = (0, 0, 255)
            for p in gt[0,frame_index,:68]:
                image[p[1]-pixel_size:p[1]+pixel_size, p[0]-pixel_size:p[0]+pixel_size,:] = (255, 0, 0)
            for p in gt[0,frame_index,71:72]:
                image[p[1]-pixel_size:p[1]+pixel_size, p[0]-pixel_size:p[0]+pixel_size,:] = (255, 255, 0)
            ax.imshow(image)
