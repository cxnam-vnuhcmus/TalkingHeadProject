...
class A2GIWSI_Discriminator(nn.Module):
    def __init__(self):
        ...
        fc_params = [512*2*2, 1024, 512, 256, 128, 1]
        fc_blocks = []
        for i in range(len(fc_params)-2):
            fc_blocks.append(nn.Linear(in_features=fc_params[i], out_features=fc_params[i+1]))
            fc_blocks.append(nn.BatchNorm1d(fc_params[i+1]))
            fc_blocks.append(nn.LeakyReLU(0.01))
        fc_blocks.append(nn.Linear(in_features=fc_params[-2], out_features=fc_params[-1]))
    ...
    def forward(self, image):
        ...
        out_encoder = out_encoder.view(out_encoder.shape[0],-1)
         
        for i in range(len(self.fc_blocks)):
            layer = self.fc_blocks[i]
            if i == 0:
                out_fc = layer(out_encoder)
            else:
                out_fc = layer(out_fc)
        out_fc = sigmoid(out_fc)
        return out_fc        
...
class A2GIWSI_GAN(nn.Module):
    ...
    def train_epoch(self, epoch):
        ...
        for step, (audio,real_img) in enumerate(self.train_dataloader):            
            ...
            fake_label = self.discriminator(fake_img)   #25,1
